...
def cutFile(self):
    ...
    cmd = "ffmpeg", "-ss", startTime, "-t", endtime, "-i", filename, "-c", "copy", filename + newName + filetype
    cmd = ' '.join(cmd)
    cmds.append(cmd)
    files.append(filename + newName + filetype)
    runcode(self)
...
def splitFile(self):
    ...
    cmd1 = ' '.join(cmd1)
    cmds.append(cmd1)
    files.append(filename + newName + "-video.mov")
    cmd2 = "ffmpeg", "-i", filename, "-c:a", "copy", "-vn", filename + newName + "-audio.mov"
    cmd2 = ' '.join(cmd2)
    cmds.append(cmd2)
    files.append(filename + newName + "-audio.mov")
    runcode(self)
...
def mergeFile(self):
    ...
    cmds.append(cmd)
    files.append(filename + newName + ".mov")
    runcode(self)
...
class Frame6(ttk.Frame):
    def __init__(self, master, *args, **kwargs):
        ...
        tk.Label(master, text="编码速度： ", font=('Arial', 9)).place(x=360, y=55)
        ...
        tk.Label(master, text="编码等级： ", font=('Arial', 9)).place(x=560, y=55)
        ...
        tk.Label(master, text="视频类别： ", font=('Arial', 9)).place(x=160, y=135)
...
class Frame7(ttk.Frame):
    def __init__(self, master, *args, **kwargs):
        ...
        '''
        self.weixinpng = tk.PhotoImage(file='weixin.png')
        about = tk.Text(master, width=112, height=38)
        about.insert('end', aboutTxt)
        about.image_create("end", image=self.weixinpng)
        about.tag_configure("center", justify='center')
        about.tag_add("center", 14.0, "end")
...
class frameClass(ttk.Frame):
    def __init__(self, master, *args, **kwargs):
        ...
        tab2 = self.tab.add(self.frame2, text=tabName[2])
        self.frame3 = tk.Frame(self.tab)
        tab3 = self.tab.add(self.frame3, text=tabName[3])
        self.frame4 = tk.Frame(self.tab)
        tab4 = self.tab.add(self.frame4, text=tabName[4])
        self.frame5 = tk.Frame(self.tab)
        tab5 = self.tab.add(self.frame5, text=tabName[5])
        self.frame6 = tk.Frame(self.tab)
        tab6 = self.tab.add(self.frame6, text=tabName[6])
